    ...
    <>
      <div className="bg-[#0A0A0F] text-white font-sans overflow-x-hidden">
        ...
        <nav className="sticky top-0 z-50 flex items-center justify-between px-6 md:px-12 py-4 border-b border-white/8 bg-[#0A0A0F]/80 backdrop-blur-md">
          ...
          <button onClick={openModal} className="flex items-center gap-2 px-4 py-2 rounded-lg border border-white/10 bg-white/5 hover:bg-white/10 text-sm font-medium transition-all">
        ...
        <section className="relative min-h-[92vh] flex items-center px-6 md:px-16 py-24 overflow-hidden">
          <div className="absolute inset-0 pointer-events-none">
            ...
            <div className="absolute right-0 top-0 w-1/2 h-full opacity-10" style={{ background: "url(\"data:image/svg+xml,%3Csvg width='600' height='800' viewBox='0 0 600 800' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cellipse cx='400' cy='300' rx='200' ry='280' stroke='%237C3AED' stroke-width='1.5' stroke-dasharray='6 4'/%3E%3Cpath d='M200 100 Q 400 200 350 500 Q 300 700 500 750' stroke='%23A78BFA' stroke-width='1' stroke-dasharray='3 5' fill='none'/%3E%3C/svg%3E\") no-repeat center/cover" }} />
          </div>
          <div className="relative max-w-2xl">
            ...
            <motion.h1 variants={fadeUp} initial="hidden" animate="show" custom={1} className="text-4xl md:text-6xl font-bold leading-[1.1] tracking-tight mb-6">
              Pare de panfletar currículos.{" "}
              <span className="bg-gradient-to-r from-brand-400 to-blue-400 bg-clip-text text-transparent">Hackeie o processo seletivo com IA.</span>
            </motion.h1>
            <motion.p variants={fadeUp} initial="hidden" animate="show" custom={2} className="text-[#94A3B8] text-lg leading-relaxed mb-8 max-w-xl">
            ...
            <motion.div variants={fadeUp} initial="hidden" animate="show" custom={3} className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
              <button onClick={openModal} className="flex items-center gap-2 px-7 py-4 rounded-xl text-base font-bold bg-gradient-to-r from-orange-500 to-brand-600 hover:opacity-90 transition-all shadow-glow text-white">
              ...
              <span className="text-xs text-white/30">Acesso instantâneo via LinkedIn ou Google</span>
            </motion.div>
          </div>
        </section>
        ...
        <section className="px-6 md:px-12 py-10 border-y border-white/6">
          ...
          <div className="flex items-center justify-center gap-6 flex-wrap">
            {["Gupy", "Workday", "LinkedIn", "Greenhouse"].map(name => (
              <div key={name} className="px-5 py-2.5 rounded-lg bg-white/4 border border-white/6 text-white/25 text-sm font-medium w-28 text-center">{name}</div>
            ))}
          </div>
        </section>
        ...
        <section className="px-6 md:px-12 py-24 max-w-6xl mx-auto">
          <motion.h2 variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }} className="text-2xl md:text-3xl font-bold text-center mb-14">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <motion.div variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }} custom={0}
              className="bg-[#1A1D27] border border-white/8 rounded-2xl p-6 flex flex-col gap-4" style={{ minHeight: 320 }}>
            ...
            <motion.div variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }} custom={2}
              ...
              <div className="mt-auto space-y-2">
                {["Palavras-chave ATS", "Tom profissional", "PDF gerado"].map(item => (
                  <div key={item} className="flex items-center gap-2 text-xs text-white/50"><span className="text-emerald-400">✓</span>{item}</div>
                ))}
              </div>
            </motion.div>
          </div>
        ...
        <section className="px-6 md:px-16 py-24 max-w-6xl mx-auto grid md:grid-cols-2 gap-12 items-center">
          <motion.div variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }}>
            ...
            <h2 className="text-3xl md:text-4xl font-bold leading-tight mb-5">Descobrindo a narrativa oculta do seu perfil</h2>
            <p className="text-[#94A3B8] leading-relaxed mb-8">A IA não inventa mentiras, ela apenas traduz a sua experiência real para a linguagem exata que o ATS foi programado para aprovar.</p>
            <button onClick={openModal} className="px-6 py-3 rounded-xl bg-brand-gradient text-sm font-semibold text-white hover:opacity-90 transition-all shadow-glow-sm">
              ...
              <div className="bg-white/5 rounded-xl p-4 border border-red-500/20">
                ...
                <div className="space-y-2">
                  {["Trabalhei com UX", "Fiz wireframes", "Usei o Figma"].map(t => (
                    <div key={t} className="h-6 bg-white/10 rounded-md flex items-center px-2 text-[10px] text-white/30">{t}</div>
                  ))}
                </div>
              </div>
              <div className="bg-brand-600/10 rounded-xl p-4 border border-brand-600/30">
                ...
                <div className="space-y-2">
                  {["UX Research & Strategy", "Design Systems (Figma)", "ATS-Optimized copy"].map(t => (
                    <div key={t} className="h-6 bg-brand-600/20 rounded-md flex items-center px-2 text-[10px] text-brand-300">{t}</div>
                  ))}
                </div>
              </div>
            </div>
        ...
        <section className="px-6 md:px-12 py-24 max-w-4xl mx-auto">
          <motion.h2 variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }} className="text-2xl md:text-3xl font-bold text-center mb-12">
        ...
        <section className="relative px-6 md:px-12 py-24 overflow-hidden">
          ...
          <motion.h2 variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }} className="text-2xl md:text-3xl font-bold text-center mb-12 relative">
          ...
          <div className="max-w-3xl mx-auto grid md:grid-cols-2 gap-6 items-stretch relative">
            ...
            <motion.div variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }} custom={1}
              ...
              <button onClick={openModal} className="w-full py-3.5 rounded-xl bg-brand-gradient text-sm font-bold text-white hover:opacity-90 transition-all shadow-glow-sm">
                Assinar Plano Pro
              </button>
            </motion.div>
          </div>
        ...
        <footer className="border-t border-white/8 py-16 px-6 text-center">
          <motion.div variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }}>
            ...
            <button onClick={openModal} className="px-8 py-4 rounded-xl bg-brand-gradient text-sm font-bold text-white hover:opacity-90 transition-all shadow-glow mb-10">
              Começar de Graça
            </button>
