...
const JobCard = ({ job }: { job: Job }) => {
  ...
  return (
    <div className="bg-surface-card border border-white/8 rounded-xl p-4 hover:border-brand-600/50 transition-all flex flex-col">
      ...
      <button
        ...
        className="mt-auto pt-3 w-full py-2.5 rounded-lg text-sm font-semibold bg-brand-gradient hover:opacity-90 text-white transition-all shadow-glow-sm"
      >
        Ver Match e Aplicar
      </button>
    </div>
    ...
    <main className="flex-1 max-w-4xl w-full mx-auto px-4 py-10">
      ...
      <div className="text-center mb-10">
        <h1 className="text-3xl font-bold text-white mb-2">
          ...
          <span className="text-brand-400">vaga ideal</span>
        </h1>
        <p className="text-white/40 text-sm">
          Busque vagas e gere um currículo adaptado com IA em segundos.
        </p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-10">
        ...
        <div className="flex-1 relative">
          <span className="absolute left-3 top-1/2 -translate-y-1/2 text-white/30 pointer-events-none text-base">
            📍
          </span>
          <input
            type="text"
      ...
      {!loading && jobs.length > 0 && (
        <>
          <p className="text-white/30 text-xs mb-4">
            {jobs.length} vagas encontradas para{" "}
            <span className="text-white/50 font-medium">"{query}"</span>
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      {!loading && searched && jobs.length === 0 && (
        <div className="text-center py-20">
          ...
          <p className="text-white/40 text-sm">
            Nenhuma vaga encontrada. Tente outro cargo ou localização.
          </p>
        </div>
      )}
    </main>
